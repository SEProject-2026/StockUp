...
export default function SettingsScreen() {
  ...
  const handleOpenCode = async () => {
      ...
      try {
        ...
        // נשאיר את זה רק לקונסול כדי שלא יקפוץ Alert למשתמש סתם
        console.error("טעינת קוד נכשלה", e); 
      } finally { 
        actions.setLoadingHomeCode(false); 
      }
    };
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      ...
      <View style={styles.main}>
        <ScreenHeader title="הגדרות" onBack={() => router.replace(`/home/${currentHomeId}`)} />
        ...
        <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
          <Section title="התראות">
            ...
            <SettingsRow 
              ...
              onPress={() => actions.setDaysModalOpen(true)} 
            />
          </Section>
        ...
        {/* הוספת הריווח התחתון סביב ה-Navbar */}
      ...
      {/* Modals */}
      <ExpiryDaysModal 
        visible={state.daysModalOpen} 
        onClose={() => actions.setDaysModalOpen(false)} 
        days={state.expiryLeadDays} 
        setDays={actions.setExpiryLeadDays} 
        onSave={actions.handleSaveExpiration} 
        loading={state.savingDays} 
        clamp={actions.clampDays} 
      />

      <HomeCodeModal 
        visible={state.homeCodeOpen} 
        onClose={() => actions.setHomeCodeOpen(false)} 
        code={state.homeInviteCode} 
        loading={state.loadingHomeCode} 
      />
      <JoinRequestsModal 
        visible={state.joinRequestsOpen} 
        onClose={() => actions.setJoinRequestsOpen(false)} 
        requests={state.joinRequests} 
        loading={state.loadingJoinRequests} 
        onAnswer={actions.handleAnswerJoinRequest} 
        processingId={state.processingRequestId} 
      />

      <SwitchHeadModal 
        visible={state.switchHeadOpen} 
        onClose={() => actions.setSwitchHeadOpen(false)} 
        members={state.homeMembers} 
        currentAdminId={state.homeMeta?.admin_id} 
        onSwitch={actions.handleSwitchHead} 
      />

      <ManageMembersModal 
        visible={state.membersOpen} 
        onClose={() => actions.setMembersOpen(false)} 
        members={state.homeMembers} 
        currentAdminId={state.homeMeta?.admin_id} 
        onRemove={actions.handleRemoveMember} 
        removingId={state.removingMemberId} 
      />

      {(state.leavingHomeLoading || state.deletingHomeLoading) && (
