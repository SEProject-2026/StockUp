...
class ReceiptParser:
    ...
    def enhance_image(self, img):
        ...
        temp_in = "page_temp_in.png"
        temp_out = "page_temp_out.png"
        cv2.imwrite(temp_in, img)

        script_dir = os.path.dirname(os.path.abspath(__file__))
        ps_script = os.path.join(script_dir, "textcleaner.ps1")
        
        # אם הסקריפט לא שם, ננסה בתיקיית השורש
        if not os.path.exists(ps_script):
             ps_script = os.path.abspath("textcleaner.ps1")

        if not os.path.exists(ps_script):
            print(f"[WARNING] textcleaner.ps1 not found at {ps_script}. Using fallback.")
        ...
        try:
            ...
            # גיבוי: אם נכשל, נחזיר את התמונה המקורית (אולי עם המרה בסיסית)
            return cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        ...
        for line_id, group in df.groupby('line_group'):
            ...
            if len(possible_barcodes) >= 1:
                if len(possible_barcodes) == 1:
                    ...
                    barcode = possible_barcodes[1] if str(possible_barcodes[0]).isdigit() and str(possible_barcodes[0]).isdigit() else possible_barcodes[0]
...
if __name__ == "__main__":
    ...
    try:
        ...
        if os.path.exists("page_temp_in.png"):
            os.remove("page_temp_in.png")
        if os.path.exists("page_temp_out.png"):
            os.remove("page_temp_out.png")
