...
class ReceiptParser:
    ...
    def _process_single_page(self, input_data, is_digital):
        ...
        group_divider = 10 if is_digital else 25
        ...
        for line_id, group in df.groupby('line_group'):
            ...
            # print(f"DEBUG Line: {full_line_text}")
            ...
            if not str(barcode).isdigit(): # המרה ל-str ליתר ביטחון
                continue
            ...
            if not is_kg_found:
                ...
                if qty_matches:
                    try: quantity = float(qty_matches[0])
                    except: quantity = 1.0
            else:
                unit_type = "ק\"ג"
