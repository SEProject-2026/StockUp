...
def save_receipt_test_case(chain_name: str, slices_info: list, final_data: dict = None):
    """
    Saves multiple slices belonging to a single receipt scan as ONE test case.
    Generates informative base names (e.g., mck1). Images get suffixes if multiple.
    ...
    try:
        ...
        for idx, sinfo in enumerate(slices_info):
            ...
            if pure_text:
                all_pure_text.append(pure_text)
                
        # 2. Save ONE combined Raw Text
        combined_raw_text = "\n".join(all_pure_text)
        raw_text_path = raw_dir / f"{base_name}_{file_suffix}.txt"
        with open(raw_text_path, "w", encoding="utf-8") as f:
            f.write(combined_raw_text)
            
        # 3. Save ONE Expected JSON
        use_data = final_data if final_data is not None else slices_info[-1].get("result", {})
        
        aggregated_items = {}
        scanned_items = use_data.get("products", [])
        ...
        expected_data = {
            "chain": use_data.get("chain", safe_chain),
