...
class DbReceiptRepository(IReceiptRepository):
    ...
    async def get_by_home(self, home_id: UUID, limit: int = 50, since: Optional[datetime] = None) -> List[ReceiptDTO]:
        stmt = (
            select(ReceiptRecordModel)
            .options(selectinload(ReceiptRecordModel.items))
            .where(ReceiptRecordModel.home_id == str(home_id))
        )
        
        if since:
            stmt = stmt.where(ReceiptRecordModel.created_at >= since)
            
        stmt = stmt.order_by(ReceiptRecordModel.created_at.desc()).limit(limit)
