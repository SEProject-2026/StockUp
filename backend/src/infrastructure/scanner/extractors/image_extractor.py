...
def _reconstruct_vision_text(response):
    ...
    for page in response.full_text_annotation.pages:
        page_width = page.width if hasattr(page, 'width') and page.width else 1000
        page_height = page.height if hasattr(page, 'height') and page.height else 1000
        ...
        for block in page.blocks:
            for paragraph in block.paragraphs:
                for word in paragraph.words:
                    ...
                    # Handle both image OCR (vertices) and PDF OCR (normalized_vertices)
                    xs = []
                    ys = []
                    box = []
                    
                    if word.bounding_box.vertices and len(word.bounding_box.vertices) > 0:
                        vertices = word.bounding_box.vertices
                        xs = [v.x for v in vertices]
                        ys = [v.y for v in vertices]
                        box = [[v.x, v.y] for v in vertices]
                    elif word.bounding_box.normalized_vertices and len(word.bounding_box.normalized_vertices) > 0:
                        vertices = word.bounding_box.normalized_vertices
                        xs = [int(v.x * page_width) for v in vertices]
                        ys = [int(v.y * page_height) for v in vertices]
                        box = [[int(v.x * page_width), int(v.y * page_height)] for v in vertices]
